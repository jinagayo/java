...
//		static int scv = outeriv;   //클래스 멤버에서 인스턴스 멤버 직접 호출불가.  객체화필요
		static int scv = new Outer2().outeriv;   
		static int scv2 = outercv;
		int siv2 = outercv;
	}
	/*
	 * 지역내부클래스에 사용되는 메소드의 지역변수는 상수화 되어야 한다.
...
//		lv++;
		class LocalInner{
			...
			void method(int num) {
				...
				System.out.println("num="+ ++num);   //내부메소드의 지역변수 100
				System.out.println("pv="+ pv);     //외부메소드의 지역변수. 0
				System.out.println("lv="+ lv);       //외부메소드의 지역변수. 100
			}
		}
