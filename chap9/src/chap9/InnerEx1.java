 ...
 * 지역 내부클래스 : 메소드 내부에서 선언된 내부클래스
 * 				 내부클래스를 선언한 메소드 내에서만 사용 가능
 * 
 * 내부클래스 특징
 * 1. 참조자료형으로 사용됨. 외부클래스명.내부클래스명
 * 2. 객체화 가능. Object클래스의 하위 클래스
 * 3. 멤버소유 가능. 생성자 구현 가능함
 * 
 * 외부클래스의 멤버임.
...
class Outer1{
	...
	void method() {			//Outer1 클래스의 멤버
		class LocalInner{   //local 내부 클래스. method()메소드 내부에서만 호출 가능
			int iv = 600;
			static  int cv = 700;
...
//		Outer1.StaticInner si = out.new StaticInner();  //오류
		...
		System.out.println("ii.MAX="+Outer1.InstanceInner.MAX);
		//Si 객체의 멤버 호출
