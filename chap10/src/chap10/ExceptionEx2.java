 ...
 * 3/0 : 예외발생 => 1256
 * 
 * catch 블럭 : try블럭 내부에서 예외발생시에 실행되는 영역
 * 
 * 다중 catch
 * - 한 개의 try블록에 여러 개의 catch 블록 존재
 *   => try블록에서 발생되는 예외가 여러 종류인 경우 예외별 예외처리 가능
 *   => 상위 예외클래스(Exception)은 catch들 중 하단에 배치해야 함 (순서 중요)
 *   
 *   예외 클래스 계층
 *   Throwable - Error
 *   		   - Exception(예외) - RuntimeException => 예외처리가 생략 가능
...
//		System.out.println(1/0);  //ArithmeticException 예외 발생
		try {
			String str = null;
			System.out.println(str.trim());  //NulPointerExeption 예외 발생
