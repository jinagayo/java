...
/*
 * 람다식 예제 : jdk8 이후에 가능
 * 함수적 인터페이스(FunctionalInterface) : 인터페이스의 추상메소드가 한 개인 인터페이스
 * 함수적 인터페이스만 람다방식으로 사용가능
 * 
 * 추상메소드가 매개변수, 리턴타입이 없는 경우의 예제
 * ()->{.......}
 * {} 내부의 문장이 한 개인 경우는 {} 생략가능
 * ()-> ...
 */
@FunctionalInterface  //함수적 인터페이스로 설정하므로 추상메소드가 한 개임을 표시
...
public class LambdaEx1 {
	...
	public static void main(String[] args) {
		...
		//2. 람다방식: 내부문장이 한 개인 경우 {} 생략 가능
		fi = () -> System.out.println("람다방식2");
		fi.method();
		execute(()->System.out.println("execute 메소드에서 호출함"));
