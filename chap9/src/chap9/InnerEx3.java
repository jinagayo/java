...
class Outer3 extends ParentOuter{
	...
	class InstanceInner{
		int iv=100;
		void method() { //내부클래스의 메소드
			int iv=300;
			/*
			 * 내부클래스의 멤버메소드에서 변수 접근 우선순위
			 * 1. 지역변수
			 * 2. 내부클래스의 멤버변수. this.멤버변수 접근 가능
			 * 3. 외부클래스의 멤버변수. 외부클래스명.this.멤버변수 접근 가능
			...
			//외부클래스의 상위 멤버 접근: 외부클래스명.this.멤버명.  외부클래스의 멤버명이 한 개인 경우
