 ...
 */
	...
	void buy (Food f){
		if(money < f.price) {
			System.out.println(f+"구매 시 잔액부족");
			return;	     //메소드 종료
		}
			// money >= f.price 경우
			...
			if(f instanceof Fruit)   //f.brix : 불가. brix멤버는 Fruit의 멤버
			System.out.println(((Fruit) f).brix + "당도 상품 구매");    //형변환. 부모타입 => 자손타입의 참조변수 : 명시적형변환
	...
	void summary() {
		...
		System.out.println("과일 구매 금액 : "+ fsum + ", 과일 구매목록 : "+ flist);
		...
		System.out.println("음료 구매 금액 : "+ dsum + ", 음료 구매목록 : "+ dlist);
		...
		System.out.println("과자 구매 금액 : "+ ssum + ", 과자 구매목록 : "+ slist);
